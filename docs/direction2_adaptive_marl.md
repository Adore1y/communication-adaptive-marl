# 方向二：通信自适应的大规模协作多智能体强化学习

## 1. 研究概述
- **研究主题**：针对 100+ 智能体协作任务，设计具备“何时通信、与谁通信、传递何种信息”自适应能力的分层 MARL 框架。
- **灵感来源**：卢宗青老师在价值分解、注意力通信、分层决策方面的研究成果。
- **目标会议**：NeurIPS、ICML、ICLR、AAMAS（亦可同步投 CoRL/ICRA MARL 轨道）。
...
4. **可扩展实现**：利用 8 × A800 的单节点资源，实现 256–512 并行环境采样 + 异步/同步训练。
...
## 7. 时间线（12 周示例）
- **W1–2**：环境搭建（仓储/SAR/编队）+ MAPPO 等基线复现。
- **W3–5**：实现分层通信结构、预算控制与置信触发。
- **W6–7**：扩展到异构智能体与混合场景，调通稳定训练。
- **W8–9**：大规模实验（多规模 + 多种子）与消融、鲁棒性测试。
- **W10–11**：整理统计结果、绘制曲线、撰写方法部分。
- **W12**：论文撰写与补充实验（若审稿需要）。
